...
##########################################################################################
  ...
  instalar_node_red(){
    ...
    if ! command -v node &>/dev/null; then
        ...
        version=$(fnm list-remote | tail -n 1)
        echo "Versión de Node.js encontrada: $version"
        
        vnum=$(echo $version | awk -F'[v.]' '{print $2}')
        echo "Versión seleccionada: $vnum"
        
        fnm use --install-if-missing $vnum
        if [ $? -eq 0 ]; then
            echo "Node.js se instaló correctamente."
        else
            echo "Hubo un error al instalar Node.js"
        fi
    else
